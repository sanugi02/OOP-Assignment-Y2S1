  ...
  <div class="flex-1 flex flex-col overflow-hidden">
    ...
    <main class="flex-1 p-6 overflow-auto">
      <div class="max-w-4xl mx-auto bg-white shadow rounded-lg p-6">
        <h2 class="text-2xl font-semibold mb-6">Update Venue Details</h2>

        <form action="${pageContext.request.contextPath}/venue?action=update" method="post" enctype="multipart/form-data">

      <input type="hidden" name="id" value="${venue.id}">
        
        <!-- Room Name -->
        <div>
          <label class="block mb-1 font-medium" for="room-name">Venue Name</label>
          <input id="room-name" name="venueName" type="text" placeholder="e.g. SEASIDE FAMILY HEAVEN" value="${venue.venueName}"
        ...
        <div>
          ...
          <c:if test="${not empty venue.image}">
            <div class="mt-4">
              <h3>Current Image:</h3>
              <img src="${pageContext.request.contextPath}/images/${venue.image}" alt="Current Venue Image" class="w-48 rounded shadow">
            </div>
          </c:if>
        ...
        <div>
          ...
          <input id="description-name" type="text" name="description"
            class="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-orange-400" value="${venue.description}">
        </div>

       <div>
          <label class="block mb-1 font-medium" for="details">Details</label>
          <input id="details" type="text" name="details"
            class="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-orange-400" value="${venue.details}">
        </div>
        ...
        <div class="pt-4">
          <button type="submit"
            ...
            <i class="fas fa-plus mr-2"></i> Update Venue
          </button>
        </div>
      </form>
    </div>
